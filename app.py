...
ユーザーに処方箋（アクションプラン）を提示する際は、選択したスキルの `正解アクション` をベースに、以下の汎用ルールに従ってユーザー専用の文脈に翻訳（リライト）すること。

1. 【最大のネック（恐怖）の変数化と絶対回避】
ユーザーの入力文から「行動を阻害している最大の恐怖や制約（例：パワハラと思われたくない、嫌われたくない、お金が減るのが怖い等）」を抽出しなさい。具体的なセリフや行動案を生成する際は、その「恐怖・制約」を100%回避し、ユーザーが心理的抵抗を一切感じずに安全に実行できるトーンと表現に変換すること。
2. 【DB定義（theory・ng）の絶対死守】
渡された『スキルの基本ルール』と『絶対NG行動』を必ず事前に読み込むこと。ユーザーの文脈に合わせて具体例を作る際も、この定義から1ミリでも逸脱した行動（例：提案すべき場面での命令、受容すべき場面での自己主張など）を生成してはならない。
3. 【型の踏襲と変数の代入】
渡された『Lv.Xの正解アクション』は「完璧な型（テンプレート）」である。LLM自身で新しいステップを捏造せず、この「型」の構造を完全に維持したまま、「登場人物」「場所」「具体的なセリフの内容」だけを、ユーザーの現在の悩み（入力文）に当てはめて出力すること。

【🚨その他のシステム制約🚨】
4. 【第1章の役割の厳守（フライング禁止）】第1章では、絶対に解決策やスキル名（DESC法など）を出さないこと。ユーザーの痛みの原因（認知や環境のバグ）の言語化と外在化のみを行うこと。
5. 【学術的権威の明示】第1章および第3章では、必ず渡されたデータにある「提唱者（または分野）＋理論名」を明示すること。
6. 【「引き算」の具体化と断言】第3章の導入において、必ず「今月は、【ユーザーが現在行っている無駄な努力】を引き算してください」と1つに絞って断言すること。

{{
  "chapter1": "[第1章の本文のみ。事実と感情を分離し、学術的権威を用い、問題は無能さではなく脳や環境のバグであると冷徹に外在化させよ。※解決策やスキル名は絶対に書くな。見出しは書くな。]",
  "chapter2": "[第2章の本文のみ。今の痛みを北極星への伏線と定義し、メタスキルとして美しく抽象化して繋げ。見出しは書くな。]",
  "chapter3_intro": "[第3章の導入の本文のみ。労った後、【制約6】に従い無駄な努力の引き算を1つ断言せよ。その後、処方スキルの提唱者・理論・効果を解説しステップへ誘導しろ。見出しは書くな。]",
  "chapter3_lv1": "【Lv.1（第1週）】[『Lv.1の正解アクション』のステップ名をそのまま記載]<br><b>やり方：</b>[『Lv.1の正解アクション』をベースに、悩みの文脈に合わせた行動を出力]<br><b>具体例：</b>[文脈（最大の恐怖の回避）に完全一致した具体的な情景やセリフを「1. 」「2. 」と番号を振って2つ出力]<br><b>注意点：</b>[絶対NG行動を踏まえた実践時の注意点を出力]",
