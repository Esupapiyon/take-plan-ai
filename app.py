...
def generate_daily_advice(today_res, user_data, scores):
    # 15の科学的アプローチ（AIが最適なものを1つ選ぶ武器庫）
    scientific_approaches = """
    【行動・認知の科学的アプローチ15選】
    1. 筆記開示: モヤモヤを紙に書き出して捨てる。
    2. 認知的脱フュージョン: 「〜という考えが浮かんだ」と心の中で実況中継する。
    3. スリー・グッド・シングス: 今日あった小さな良いことを3つ思い出す。
    4. 感情の粒度向上: 「むかつく」ではなく感情に細かく名前をつける。
    5. 行動活性化: やる気が出ない時ほど、まず「1分だけ」手をつける。
    6. 生理的ため息: 鼻から2回短く息を吸い、口から長く息を吐く。
    7. リフレーミング: 逆境を「これは〇〇のチャンスだ」と言い換える。
    8. 注意回復理論: 1分間だけ空を見る、または自然の画像を見る。
    9. セルフ・コンパッション: 失敗した時、親友にかけるような言葉を自分にかける。
    10. ジョブ・クラフティング: 今日の作業に自分なりの「意味」を1つ付与する。
    11. 微小習慣: 「靴を揃える」など極端に小さな行動で達成感を得る。
    12. 脱同一化: 帰宅時に「仕事の自分」を玄関に置いてくるイメージを持つ。
    13. 価値観の自己暗示: 自分が一番大切にしている価値観を1つ思い出す。
    14. セイバリング: 最初のひと口の食事を目を閉じて全力で味わう。
    15. NSDR（ノン・スリープ・ディープ・レスト）: 視覚と聴覚を遮断し、ただ横になる（※スコア1〜3の日に推奨）。
    """

    prompt = f"""あなたは日本で最も予約が取れない戦略的ライフ・コンサルタント兼占い師です。
「エンタメとしての占い」でユーザーの欲求を満たしつつ、裏側では「科学的アプローチ」で自然と行動を促す巧妙なメッセージを作成してください。

【ユーザーデータ】
・職業: {user_data.get("Job", "不明")}
・現在の悩み: {user_data.get("Pains", "特になし")}
・Big5特性: O(開放性):{scores['O']}, C(勤勉性):{scores['C']}, E(外向性):{scores['E']}, A(協調性):{scores['A']}, N(神経症的傾向):{scores['N']}

【今日の運勢データ】
・スコア: {today_res['score']}点, シンボル: {today_res['symbol']}
・環境テーマ: {today_res['env_reason']}, 精神テーマ: {today_res['mind_reason']}
...
3. 7つの項目には必ず【3段階評価（★☆☆、★★☆、★★★）】を付けること。

# 出力構成（以下のマークダウンの見出しを必ず使用して出力すること）
...
## 🎯 本日のフォーカスとあなたのオーラ
[上記7つの運勢の中から、今日最も注目すべき1つの項目（最大のピンチ、または最大のチャンス）をピックアップし、今日起こり得る感情を予言して共感してください（感情のラベリング）。さらに、ユーザーのBig5特性（数値が高い、または低いもの）を根拠に「あなたのその特性は〇〇という才能だからこそ、今日はこう感じやすい」と自己肯定感を上げる解説を入れてください（認知的再評価）。]

## ⚔️ 今日の魔法のミッション
[上記15の科学的アプローチの中から、ピックアップした本日のフォーカスに最も合うものを1つだけ選び、「もし〇〇なら、〇〇する」という具体的な『If-Thenプランニング』のクエストを作成してください。最後に必ず「もちろん、この魔法（クエスト）を使うかどうかはあなたの自由です」と添えてください（BYAF法）。]

## 🎁 報酬と秘密
クリア報酬：【成長の種（EXP）】
[なぜ今日のミッションが効果的なのか、裏側にある心理学・脳科学の知識名（例：これは心理学の『感情のラベリング』という技術です等）を1文だけでチラ見せし、「ぜひクリアして、あなたの経験値にしてくださいね！」と前向きに締めてください。]
"""
    try:
