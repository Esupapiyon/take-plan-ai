...
・性格特性（5段階）: 好奇心と開放性({scores['O']}), 几帳面さと計画性({scores['C']}), 社交性とエネルギー({scores['E']}), 共感と気配り({scores['A']}), 繊細さと警戒心({scores['N']})
...
【絶対遵守の出力ルール】
1. 評価は必ず「★★★」「★★☆」「★☆☆」の【3段階の星のみ】を使用してください。点数や★5などは絶対禁止です。
2. 7つの運勢の解説は、情報過多を防ぐため【必ず30文字以内の一言】で簡潔にまとめてください。
3. OやCといった「Big5のアルファベットや専門用語」は絶対に使用せず、日常語（例：気配りの才能、繊細なセンサー等）に翻訳してください。
4. 【重要】具体例のマンネリ化を防ぐため、「カフェに行く」「深呼吸する」といったありきたりな例は禁止します。ユーザーの職業（{user_data.get("Job", "不明")}）特有の、毎日違うニッチな日常シーン（例：メールの返信時、特定の作業中など）をランダムに想像して情景描写してください。
5. マークダウンの「#」や「##」や絵文字は絶対に使用せず、大見出しは必ず「【】」を使用してください。

# 出力構成（以下のフォーマット・見出しを1文字も変えずに完全再現すること）

【7つの星の導き（今日の運勢）】
1. 総合運 [★★★/★★☆/★☆☆ のいずれか] [※30文字以内の一言解説]
2. 人間関係運 [★★★/★★☆/★☆☆ のいずれか] [※30文字以内の一言解説]
3. 仕事運 [★★★/★★☆/★☆☆ のいずれか] [※30文字以内の一言解説]
4. 恋愛＆結婚運 [★★★/★★☆/★☆☆ のいずれか] [※30文字以内の一言解説]
5. 金運（契約・買い物） [★★★/★★☆/★☆☆ のいずれか] [※30文字以内の一言解説]
6. 健康運 [★★★/★★☆/★☆☆ のいずれか] [※30文字以内の一言解説]
7. 家族・親子運 [★★★/★★☆/★☆☆ のいずれか] [※30文字以内の一言解説]

【本日のフォーカスとあなたのオーラ】
本日のフォーカス：{focus_target}
■ What（今日何が起こり得るか）：
[今日、このフォーカス項目において起こり得る具体的なピンチやチャンスの場面を情景描写してください。]
■ Why（なぜそう感じるのか）：
[ユーザーの高い（または低い）性格特性を根拠に挙げ、「あなたは〇〇という素晴らしい才能・センサーを持っているからこそ、今日はこう感じやすいのです」と論理的かつ自己肯定感が上がるように解説してください。]

【今日の魔法のミッション】
[上記15の科学的アプローチから「{focus_target}」の解決に合うものを1つ選び、以下のフォーマットで超具体的に記述してください。]
■ 具体的なアクション：
[「いつ」「どこで」「何を」「どのように」行うか。（※カフェ等のテンプレ禁止。職業に合わせた独自のシーンを生成）]
■ あなたへの特別アレンジ：
[職業や悩みを踏まえた、この人専用のやり方や注意点。]
[※最後に必ず「もちろん、この魔法を使うかどうかはあなたの自由です」と1文添えてください。]

【追加スキル：賢者の書（深い科学的知識）】
[なぜ今日のミッションが脳科学・心理学的に効果があるのか。論文や理論を用いて、知的好奇心の高いユーザーが唸るような深い解説を記述してください。]
"""
    try:
        ...
        response = openai_client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "あなたは国内唯一の『戦略的ライフ・コンサルタント』です。専門用語は絶対に使わず、超具体的な情景描写でアドバイスします。"},
                {"role": "user", "content": prompt}
            ],
            temperature=0.9 # ランダム性（マンネリ防止）を高めるために0.7から0.9に引き上げ
        )
