...
[必ず以下のフォーマット通りに改行し、3つ出力してください。セリフは日常会話で使う極めて自然な口語（例：「これ、結論から言うと〜」等）にしてください。不自然な敬語や説明口調のセリフは禁止です。]

① 【具体的なアクションと自然なセリフ】
・なぜ有効か：[{target_name}さんのどのような心理的欲求・性格に刺さるからか、理由を解説]
・どうなるか：[これを実行すると、結果的に相手がどう変化し、あなたにどんなベネフィットがあるか]
