...
以下の【ユーザーの分析データ】をインプットとしますが、出力する文章には「癸酉」「石門星」「天将星」「Big5」「開放性」といった【専門用語は絶対にそのまま出力しないでください】。すべて「柔軟で人に合わせるのがうまい」「実は社長の気質」「好奇心おばけ」など、誰でもクスッと笑えてスッと理解できる日常的な言葉に翻訳してください。
...
# 出力ルール
・「第○章」という見出しは使用禁止。指定された見出しのみを使用すること。
・全体的にユーモアと例え話を交え、抽象的な表現を避けること。
・欠点は「伸びしろ」や「愛嬌」としてマイルドかつ面白く伝えること。

# 出力構成（以下の通りに出力してください）

## 宿命と現実
宿命：[※専門用語を排除した、本来の気質を一言で]
現実：[※専門用語を排除した、現在の性格を一言で]
→【ここにユーザーの現状と本質を表す、最大級にキャッチーでユーモアのある一言キャッチコピー】

## 持っている星
※ユーザーの算命学の星とBig5のデータから導き出される「具体的な特徴やクセ」を15個抽出してください。（例：「上下関係が厳しい星」「実はプライドがエベレスト級の星」「褒められるとすぐ調子に乗る星」「胃腸が弱りやすい星」など）。
※抽出した15個の星を、Markdownの表形式（3列×5行）で美しく出力してください。

## 生まれ持った宿命と現在の性格
本来あなたがどんな素晴らしい才能やエンジンを持って生まれてきたのか、そして現在、社会でどんな風に振る舞っているのかを、日常的な例え話を交えてわかりやすく解説してください。

## 生きづらい正体とうまくいく考え方
本来の宿命と現在の性格の間にどんな「ズレ」が生じているか（なぜ疲れるのか、なぜ評価されないのか）をズバリ指摘し、どう考え方を変えればスッと楽になるのかをアドバイスしてください。

## 仕事・勉強
【〇〇〇〇〇〇〇〇〇〇〇〇〇〇】（※冒頭に、ゲッターズ飯田風の目を引く一言キャッチコピーを配置。例：好きなことで起業すると大金持ちになれるかも）
このジャンルにおける性格、利点、欠点を解説し、最後に以下の形式で3つのアドバイスを提示してください。
・[アドバイス1]
・[アドバイス2]
・[アドバイス3]

## 恋愛・結婚
【〇〇〇〇〇〇〇〇〇〇〇〇〇〇】（※冒頭に一言キャッチコピー。例：真面目に考えすぎるところがある高嶺の花）
このジャンルにおける性格、利点、欠点を解説し、最後に以下の形式で3つのアドバイスを提示してください。
・[アドバイス1]
・[アドバイス2]
・[アドバイス3]
...
## お金
【〇〇〇〇〇〇〇〇〇〇〇〇〇〇】（※冒頭に一言キャッチコピー。例：健全に稼いで経済を回す側になるといい）
このジャンルにおける性格、利点、欠点を解説し、最後に以下の形式で3つのアドバイスを提示してください。
・[アドバイス1]
・[アドバイス2]
・[アドバイス3]
...
## 健康
【〇〇〇〇〇〇〇〇〇〇〇〇〇〇】（※冒頭に一言キャッチコピー。例：ヘルシー思考なのに、たまにジャンクフードを爆食いする）
このジャンルにおける性格、利点、欠点（出やすい不調など）を解説し、最後に以下の形式で3つのアドバイスを提示してください。
・[アドバイス1]
・[アドバイス2]
・[アドバイス3]

## 5大欲求
以下の5つの欲求について、データの強弱から判定し、改行して解説文を書いてください。
・引力・金銭欲（人やお金を引き寄せ所有したい欲）：[判定結果（例：意外と強欲 / 悟りを開いている 等）]
　[ここにユーモアのある解説文]
・自我・自己実現欲（自分のこだわりを貫きたい欲）：[判定結果]
　[ここにユーモアのある解説文]
・快楽・表現欲（食欲・性欲・遊びなど楽しむ欲）：[判定結果]
　[ここにユーモアのある解説文]
・支配・達成欲（他者をコントロールし達成したい欲）：[判定結果]
　[ここにユーモアのある解説文]
・探求・知恵欲（知識を得て自由に考えたい欲）：[判定結果]
　[ここにユーモアのある解説文]
