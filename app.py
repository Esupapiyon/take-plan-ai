...
【出力構成と指定の絶対ルール】（※以下のPhase1〜4の順序と構成を厳守すること）

■ Phase 1：直感と共感（視覚と右脳に刺す）
1. 宿命と現実（キャッチコピー）
   宿命：[算命学ベースの本質を、静かで重みのある一行で表現]
   現実：[Big5ベースの現在の状態を、内面的な葛藤を含めた唯一無二の一行で表現]
   ※【重要】年齢、職業、目標金額等の「表面的なステータス」は一切使用禁止。
   ※「現実：」の行は、必ずHTMLタグ <h2> で最大サイズで表示すること。

2. あなたの中に眠る「7つのパラドックス（矛盾）」
   算命学とBig5の要素が衝突して生まれる、ユーザー特有のエッジの効いた矛盾（星）を「7つ」だけ、箇条書きで短く表現すること。

■ Phase 2：客観的データ（論理と左脳を納得させる）
3. あなたを駆動する「5大欲求パラメーター」
   [自由・探求欲求] [支配・達成欲求] [愛・つながり欲求] [生存欲求] [快楽・表現欲求] の5つについて、それぞれ0〜100%の数値を出し、各1〜2行で「なぜその数値なのか（宿命×現在のソフトウェアの掛け合わせ）」を解説すること。

■ Phase 3：深淵な自己理解（占い×科学の完全融合）
4. なぜ今、あなたは「生きづらい」のか（システムエラーの解剖）
   この章が最大の山場です。1000〜1500文字程度で厚く語ること。以下の3段構成で論理を展開してください。
   【ハードウェアの設計（算命学）】本来どのような器（役割・精神性）として設計されているか。
   【現在のソフトウェア（Big Five）】現在、どのような性格・行動特性のプログラムがインストールされて動いているか。
   【発生しているバグ】ハードウェアとソフトウェアのミスマッチにより、現在どのような「システムエラー（焦り、停滞感、生きづらさ）」が起きているか。

■ Phase 4：現実への実装（自走のための処方箋）
5. 領域別・バグの現れ方とハック（攻略）法
   [仕事] [人間関係] [恋愛] [金運] [健康] の5領域について。
   Phase 3の内容は繰り返さず、各領域での「バグの具体的な現れ方」と、それを乗り越えるための「明確なハック（攻略）法」をそれぞれ3〜4行で提示すること。

6. 結びの言葉
   レポートの最後は、バグの修正に向けたユーザーの背中を静かに押す、パーソナライズされたエールを3行程度の短い文で出力して締めくくること。

【最重要】出力指示：情報の重複（同じ説明の繰り返し）を避け、各フェーズの役割を明確にすること。文章は非常に長くなりますが、絶対に途中で途切れないように、最後まで完全に書き切ってください。""",
                messages=[
