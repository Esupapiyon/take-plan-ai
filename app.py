...
・その後、スキルの提唱者・理論・効果を解説し、ステップへ誘導する自然な文章を生成すること。※JSON内の[ ]で書かれた指示文そのものを出力してはならない。
...
{{
  ...
  "chapter3_intro": "{selected_empathy} 今月の引き算として、【ユーザーが現在行っている無駄な努力】を完全にストップ（＝やめる決断）してください。[ここに、処方スキルの提唱者・理論・効果を自然な文章で解説し、アクションステップへ誘導するテキストを生成すること。※このカッコ内の指示文自体は絶対に出力しないこと。見出しは書くな。]",
