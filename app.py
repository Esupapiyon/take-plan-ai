...
"""
                            try:
                                ...
                                # ==========================================
                                # ▼ 追加：Skill_History（図鑑）への保存ロジック
                                # ==========================================
                                try:
                                    import json
                                    header_row = sheet.row_values(1)
                                    if "Skill_History" in header_row:
                                        history_idx = header_row.index("Skill_History") + 1
                                        ...
                                        # ※注意：LLMのJSON出力で指定されている「スキルID」のキー名に合わせてください
                                        # （例：'skill_id' や 'selected_skill' など）
                                        prescribed_skill = result_data.get('skill_id') 
                                        
                                        if prescribed_skill:
                                            # 図鑑に表示する用のマニュアルHTMLを組み立て
                                            manual_html = f"""
        ...
        try:
            ...
            if "Skill_History" in header_row:
                ...
                if history_str:
                    ...
                    # 辞書のキー（"SKILL_01"など）をリスト化して解放済みスキルとして扱う
                    user_unlocked_skills = list(user_skill_manuals.keys())
        except Exception as e:
            st.warning("図鑑データの読み込みに失敗しました。")
