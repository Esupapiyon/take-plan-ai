...
2. 【NGワードと専門用語の完全禁止】「カフェ」「深呼吸」「散歩」等の陳腐な表現や、「Big5」「O」「C」「E」「A」「N」といった専門用語・アルファベットは【絶対に使用禁止】。
3. 【見出しの禁止】JSONのvalue（文章内）に見出し文字は絶対に書かない。
4. 【指定フォーマットへの完全代入】"mission"の各項目は、以下の【⚔️本日の固定変数】を使用し、【構成指定】の通りに厳格に出力すること。LLMのアドリブで勝手なタスクをでっち上げることは一切許さない。
...
・"action": "毎日必ずすること、たとえば「[TRIGGER_CONTEXT]」で、[MISSION_TITLE]を実行してみませんか？もし疲れていてできなくても、[TINY_HABIT]で立派なクリアです。" をベースにし、ユーザーの文脈に合わせて自然な日本語に微調整して出力。
・"benefit": "これは心理学の『[WEAPON_NAME]』を活用する魔法です。[WEAPON_THEORY]" をベースに、中学生でもわかる言葉に翻訳して出力。
