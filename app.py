...
def generate_report_prompt(sanmeigaku, scores):
    ...
    prompt = f"""あなたは、東洋哲学の最高峰「算命学」と、最新の行動心理学「Big5（パーソナリティ分析）」を完全に統合した、国内唯一の『戦略的ライフ・コンサルタント』です。

以下の【ユーザーの分析データ】をもとに、ユーザーが自分自身の取扱説明書として何度も読み返したくなるような、感動的で説得力のある「極秘レポート完全版」を作成してください。

# ユーザーの分析データ
[算命学（本来の宿命・理想）]
・日干支（10タイプの本質）：{sanmeigaku['日干支']}
・主星（価値観のベース）：{sanmeigaku['主星']}
・持っている星（エネルギー・12星）：初年[{sanmeigaku['初年']}]、中年[{sanmeigaku['中年']}]、晩年[{sanmeigaku['晩年']}]、最晩年[{sanmeigaku['最晩年']}]

[Big5（現在の深層心理・現実）] ※各1.0〜5.0
・開放性 (O): {scores['O']}
・勤勉性 (C): {scores['C']}
・外向性 (E): {scores['E']}
・協調性 (A): {scores['A']}
...
# 絶対遵守の出力ルール
1. 【文章スタイル】単なる箇条書きの羅列は絶対に避け、文脈が繋がった「読み物（ストーリー）」として出力してください。語り口は、専門的でありながらも、ユーザーに寄り添う温かく前向きなトーン（「〜です」「〜でしょう」）で統一してください。
2. 【表現の変換】ユーザーの欠点や弱点は、決して強く否定せず「伸びしろ」や「才能の裏返し」としてマイルドかつ論理的に表現してください。
3. 【文字数】全体で2000文字〜2500文字程度の充実したボリュームにしてください。

# 出力構成（以下の章立てで文章を展開してください）

## タイトル：【完全版】あなたの裏ステータス攻略レポート

## 第1章：あなたが生まれ持った「宿命」と持っている星（理想）
算命学のデータを基に、ユーザーの根源的な本質、才能、そして人生の各年代（初年・中年・晩年・最晩年）で発揮されるエネルギー（星）の特性を、ワクワクするような文章で解説してください。

## 第2章：現在の深層心理と行動のクセ（現実）
Big5のスコアを基に、現在のユーザーが社会でどのような思考や行動をとっているか、具体的な情景が浮かぶように解説してください。

## 第3章：生きづらさの正体（宿命と現実のギャップ分析）
第1章（本来の自分）と第2章（今の自分）のデータを比較し、そこにどのような「ズレ（摩擦）」が生じているかを論理的に分析してください。このギャップこそがモヤモヤの原因であることを伝え、ユーザーの心を軽くしてください。

## 第4章：あなたの基本性格と「うまくいく考え方」
ギャップを踏まえた上で、以下の項目について具体的なアドバイスを文章で記述してください。
・性格、長所と短所（短所はポジティブに変換）、向き不向き
・人間関係における最適な距離感と、ストレスを溜めないための「考え方のクセ」の直し方

## 第5章：ジャンル別・人生最適化アルゴリズム
以下の4つのジャンルについて、算命学の宿命とBig5の現実を掛け合わせた具体的な行動指針を書いてください。
・仕事・勉強（才能が最も評価される環境とポジション）
・お金・投資・買い物（無意識のブロックや浪費のクセとその対策）
・恋愛・パートナーシップ（本能的に惹かれる相手と、長期的な幸せを築ける相手の違い）
・健康・美容（ストレスが出やすい身体の部位や、最適なリフレッシュ方法）

## 第6章：あなたの5大欲求パラメーター
ユーザーのデータから、以下の5つのオリジナル欲求の強さ（非常に強い/強い/標準/控えめ等）を判定し、それぞれの欲求との上手な付き合い方を解説してください。
1. 自我・自己実現欲（自分のこだわりを貫きたい欲求）
2. 快楽・表現欲（食欲・性欲・遊び・創作など楽しむ欲求）
3. 引力・金銭欲（人やお金を引き付け、所有したい欲求）
4. 支配・達成欲（他者をコントロールし、目標を達成したい欲求）
5. 探求・知恵欲（知識を得て、自由に考えたい欲求）
