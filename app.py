...
SECRET_SKILLS.update({
    ...
    "SKILL_02": {
        ...
        "ai_guardrail": "【翻訳時の絶対ルール】S（提案）を生成する際、「〇〇してください」「〇〇を守れ」といった相手に行動を強制する命令形への翻訳は絶対NG。必ず「〇〇しませんか？」「〇〇の時間をカレンダーに入れませんか？」など、相手がYes/Noで選べる代替案・仕組みの提案として出力すること。",
        "action_steps": {
    ...
    "SKILL_14": {
        ...
        "ai_guardrail": "【翻訳時の絶対ルール】相手に「アドバイス」や「解決策」を提示するセリフを生成することは絶対NG。ユーザーのセリフは必ず「相手の感情や事実のオウム返し」のみで構成すること。",
        "action_steps": {
    ...
    "SKILL_19": {
        "name": "タイム・ボクシング",
        "desc": "「完璧に仕上げよう」とダラダ時間をかけてしまう悪癖を消し去り、圧倒的なスピードでタスクを終わらせることができる。",
    ...
    "SKILL_30": {
        ...
        "theory": "ピア・プレップレッシャーの逆利用や、自らペナルティを設定することで、現在バイアスに負ける「未来の自分」の選択肢をあらかじめ制限する行動経済学の拘束技術。",
...
"""
                        ...
                        with st.spinner(f" 処方スキル【{skill_data['name']}】に基づき、月次戦略レポートを生成中...（STEP 2/2）"):
                            ...
                            # ==========================================
                            # 【STEP 2】CoT（内部思考）強制搭載・要素完全維持プロンプト
                            # ==========================================
...
・AI翻訳専用ガードレール(ai_guardrail): {skill_data.get('ai_guardrail', '特になし')}
...
【🚨絶対遵守：アクション生成における汎用翻訳ルール🚨】
1. 【最大のネック（恐怖）の変数化と絶対回避】ユーザーの入力から「行動を阻害している最大の恐怖や制約」を抽出し、その恐怖を100%回避する安全なトーンに変換すること。
2. 【DB定義の絶対死守】『スキルの基本ルール』『絶対NG行動』および『ai_guardrail』を必ず順守し、そこから1ミリでも逸脱した行動を生成しないこと。
3. 【型の踏襲】渡された『Lv.Xの正解アクション』の型を維持し、「登場人物」「場所」「具体的なセリフ」だけをユーザーの文脈に当てはめて出力すること。独自のアクションを捏造しないこと。
4. 【フレームワーク要素の完全維持】抽出したスキルが複数の構成要素（例：DESC、WOOP等）を持つ場合、1つの具体例の中に【すべての要素】を欠落なく含有させた完全なパッケージとして出力すること。要素を複数の具体例に分割・省略することは厳禁とする。

【🚨出力フォーマットと章ごとの絶対ルール🚨】
出力は必ず以下のJSON構成とし、各章の役割を絶対に混同（フライング）させないこと。
最初に必ず "0_internal_thought"（内部思考）を行い、ルールや恐怖を確認してから文章を生成すること。

■ 第1章：痛みの正体（バグの特定）
・【絶対ルール】「解決策」や「スキル名」を絶対に記載しないこと。原因の科学的特定と外在化のみを行うこと。
■ 第2章：北極星への伏線（パラダイムシフト）
・痛みを未来への伏線として意味づけすること。
■ 第3章：今月の引き算と継続フレームワーク
・【絶対ルール】ここで初めて極秘ライブラリから選択した「スキル名」を提示すること。
・導入部分では、指定された固定の「労いの言葉」から始まり、続けて「今月の引き算として、【ユーザーが行っている無駄な努力】を完全にストップ（＝やめる決断）してください」と断言すること。

{{
  "0_internal_thought": "[内部思考ログ。ユーザーの最大の恐怖は何か？ ai_guardrail の指示は何か？ 要素の分割を防ぐにはどう構成すべきか？ これから書く文章で絶対にやってはいけない表現は何か？ を論理的に分析せよ。※画面には出力されない]",
  "chapter1": "[第1章の本文のみ。解決策やスキル名は絶対に出さず、痛みの原因（バグ）の科学的特定のみを冷徹に行う。見出しは書くな。]",
  "chapter2": "[第2章の本文のみ。痛みをメタスキルの獲得という伏線として意味づけせよ。見出しは書くな。]",
  "chapter3_intro": "{selected_empathy} 今月の引き算として、【ユーザーが現在行っている無駄な努力】を完全にストップ（＝やめる決断）してください。続けて、処方スキルの提唱者・理論・効果を解説してステップへ誘導しろ。見出しは書くな。",
  "chapter3_lv1": "【Lv.1（第1週）】[『Lv.1の正解アクション』のステップ名]<br><b>やり方：</b>[文脈に合わせた行動を出力]<br><b>具体例：</b>[恐怖の回避と全要素の含有に完全一致した具体的な情景やセリフを「1. 」「2. 」と番号を振って2つ出力]<br><b>注意点：</b>[絶対NG行動・ai_guardrailを踏まえた実践時の注意点を出力]",
  "chapter3_lv2": "【Lv.2（第2〜3週）】[『Lv.2の正解アクション』のステップ名]<br><b>やり方：</b>[微調整して出力]<br><b>具体例：</b>[文脈に合わせた具体的な情景や行動を「1. 」「2. 」と番号を振って2つ出力]<br><b>注意点：</b>[注意点を出力]",
  "chapter3_lv3": "【Lv.3（第4週）】[『Lv.3の正解アクション』のステップ名]<br><b>やり方：</b>[微調整して出力]<br><b>具体例：</b>[指定された環境下で実際に口に出すリアルなセリフや行動を「1. 」「2. 」と番号を振って2つ出力]<br><b>注意点：</b>[注意点を出力]"
}}
"""
                            try:
                                response = openai_client.chat.completions.create(
                                ...
                                # ※ '0_internal_thought' は意図的にHTMLに組み込まず隠蔽する
                                # ==========================================
                                html_output = f"""
