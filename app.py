...
"""
                        ...
                        with st.spinner(f" 処方スキル【{skill_data['name']}】に基づき、月次戦略レポートを生成中...（STEP 2/2）"):
                            ...
                            # ==========================================
                            # 【STEP 2】CoT（内部思考）強制搭載・完全制御プロンプト
                            # ==========================================
...
2. 【DB定義の絶対死守】『スキルの基本ルール』『絶対NG行動』および『ai_guardrail』を必ず順守し、そこから1ミリでも逸脱した行動（提案すべき場面での命令など）を生成しないこと。
3. 【型の踏襲】渡された『Lv.Xの正解アクション』の型を維持し、「登場人物」「場所」「具体的なセリフ」だけをユーザーの文脈に当てはめて出力すること。

【🚨出力フォーマットと章ごとの絶対ルール🚨】
出力は必ず以下のJSON構成とし、各章の役割を混同しないこと。
...
・【絶対ルール】ここで初めてスキル名を提示し、導入で「今月は、【ユーザーが行っている無駄な努力】を引き算してください」と1つ断言すること。

{{
  "0_internal_thought": "[内部思考ログ。ユーザーが最も恐れていることは何か？ ai_guardrail の指示は何か？ これから書く文章で絶対にやってはいけない表現は何か？ を論理的に分析せよ。※この内容は画面には出力されない]",
  "chapter1": "[第1章の本文のみ。解決策やスキル名は絶対に出さず、痛みの原因の科学的特定のみを冷徹に行う。見出しは書くな。]",
  "chapter2": "[第2章の本文のみ。痛みをメタスキルの獲得という伏線として意味づけせよ。見出しは書くな。]",
  "chapter3_intro": "[第3章の導入の本文のみ。無駄な努力の引き算を1つ断言し、ここで初めて処方スキルの提唱者・理論・効果を解説してステップへ誘導しろ。見出しは書くな。]",
  "chapter3_lv1": "【Lv.1（第1週）】[『Lv.1の正解アクション』のステップ名]<br><b>やり方：</b>[文脈に合わせた行動を出力]<br><b>具体例：</b>[最大の恐怖の回避に完全一致した具体的な情景やセリフを「1. 」「2. 」と番号を振って2つ出力]<br><b>注意点：</b>[絶対NG行動・ai_guardrailを踏まえた実践時の注意点を出力]",
