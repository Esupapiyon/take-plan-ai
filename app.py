...
【🚨絶対遵守の出力ルール🚨】
1. AIへの指示文（「慈悲を与えよ」「提案せよ」など）をそのまま出力する「オウム返し」は絶対禁止。
2. JSONの値の中に、「第1章：痛みの正体」や「第3章の導入」といった【見出しの言葉】は絶対に含めるな。いきなり本文から書き始めろ。
3. 専門用語（Big5、算命学、心理学理論名）は【一切使用禁止】。
4. 純粋なJSONテキストのみを出力せよ（Markdownブロックは不要）。
