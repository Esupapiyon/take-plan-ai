...
【絶対遵守の出力ルール】
1. OやCといった「Big5のアルファベットや専門用語」は絶対に使用せず、「気配り上手な才能」「繊細なセンサー」など日常語に翻訳すること。
2. 抽象的な表現（新しいアイデア、賢い判断など）は禁止。「通勤電車の中で〇〇について考える」など、情景が浮かぶレベルで超具体的に書くこと。
3. マークダウンの「#」や「##」や絵文字は絶対に使用せず、見出しは必ず「【】」を使用すること。

# 出力構成（以下の【】の見出しを必ず使用して出力すること）
...
【本日のフォーカスとあなたのオーラ】
[必ず指定された「{focus_target}」について解説してください。今日起こり得る具体的なピンチやチャンスの場面を情景描写し、ユーザーの性格特性（例：気配りが高いからこそ〜）を根拠にして、「それはあなたの〇〇という素晴らしい才能の裏返しです」と自己肯定感を上げてください。]

【今日の魔法のミッション】
[上記15の科学的アプローチから「{focus_target}」の解決に合うものを1つ選び、以下のフォーマットで超具体的に記述してください。
・具体的な手順：「いつ」「どこで」「何を」「どのように」行うか。（例：ランチ休憩中、スマホを置いて目を閉じ…等）
・あなたへのアレンジ：職業（{user_data.get("Job", "不明")}）や悩み（{user_data.get("Pains", "特になし")}）を踏まえた、この人専用のやり方や注意点。
最後に必ず「もちろん、この魔法を使うかどうかはあなたの自由です」と添えてください。]

【追加スキル：賢者の書（深い科学的知識）】
[なぜ今日のミッションが脳科学・心理学的に効果があるのか。論文や理論（例：デフォルト・モード・ネットワーク、ツァイガルニク効果など）を用いて、知的好奇心の高いユーザーが唸るような深い解説を記述してください。]
"""
    try:
