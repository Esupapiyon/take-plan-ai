                            ...
                            try:
                               　response = anthropic_client.messages.create(
                                  ...
                                  　　 temperature=0.7,
                                   ...
                                   　　messages=[
                                       　　　{"role": "user", "content": prompt + "\n\n【重要】必ずJSON形式のみで出力し、文章内の改行は必ず \\n でエスケープしてください。最初の挨拶や ```json などのマークダウン記号は絶対に含めないでください。"}
                                   　　]
                               　)
                               　
                               ...
                               　if start_idx != -1 and end_idx != -1:
                                     ...
                                     import json
                                     # strict=False を入れることでClaude特有の「生の改行バグ」を許容して強制パースする
                                     result_data = json.loads(clean_report, strict=False)
                            　　　else:
                                     raise ValueError(f"AIがJSONを出力しませんでした。生データ: {raw_report[:50]}...")
