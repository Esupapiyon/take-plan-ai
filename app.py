...
if p_mode in ["portal", "report"] and st.session_state.line_id:
    ...
    if "app_opened_logged" not in st.session_state:
        ...
        track_insight(actual_id, "App", "Open", "Portal_View")
                ...
                if new_north_star.strip():
                    with st.spinner("保存中..."):
                        ...
                        if success:
                            ...
                            track_insight(actual_id, "North_Star", "Update", new_north_star[:20]) 
                            time.sleep(1)
                            st.rerun()
                        else:
                            st.error(msg)
                        ...
                        if new_profession and new_focus:
                            ...
                            with st.spinner(" あなたの決断を受信し、全戦略を再構築しています..."):
                                ...
                                if success:
                                    ...
                                    track_insight(actual_id, "Status", "Update", detail_info)
                                    time.sleep(1)
                                    st.rerun()
                                else:
                            ...
                            with st.status(" ターゲットの深層心理を解析中...", expanded=True) as status:
                                ...
                                if not success:
                                    ...
                                    try:
                                        ...
                                        actual_id = st.session_state.get("line_id", st.session_state.get("user_id", "unknown_user"))
                                        track_insight(actual_id, "Radar", "Search")
...
"""
                            try:
                                ...
                                actual_id = st.session_state.get("line_id", st.session_state.get("user_id", "unknown_user"))
                                track_insight(actual_id, "Monthly_Strategy", "Execute")
    ...
    with tab6:
        ...
        if "library_view_logged" not in st.session_state:
             actual_id = st.session_state.get("line_id", st.session_state.get("user_id", "unknown_user"))
             track_insight(actual_id, "Skill_Library", "View", "")
