...
"""
    ...
    with tab6:
        ...
        # 現在のユーザーが過去に処方されたスキルのキー（例: "SKILL_01"）をリスト形式で取得してください。
        # ここでは仮置きとして空リスト（すべてロック状態）にしています。
        # ==========================================
        if 'user_unlocked_skills' not in locals():
            user_unlocked_skills = [] # 例: ["SKILL_02", "SKILL_15"] のようにDBから取得したリストを入れる
            
        # タブ5で生成された「あなた専用の具体的なやり方」のテキスト（辞書形式を想定）
        if 'user_skill_manuals' not in locals():
            user_skill_manuals = {} # 例: {"SKILL_02": "<div>タブ5で生成されたHTMLテキスト...</div>"} 

        # --------------------------------------------------
        # ▼ ルート定義（ルートFまで全網羅）
        # --------------------------------------------------
        routes = {
            "ルートA【対人・交渉】": [f"SKILL_{i:02d}" for i in range(1, 16)],
            "ルートB【キャリア・タスク】": [f"SKILL_{i:02d}" for i in range(16, 31)],
            "ルートC【メンタル・自己】": [f"SKILL_{i:02d}" for i in range(31, 46)],
            "ルートD【お金・リソース】": [f"SKILL_{i:02d}" for i in range(46, 61)],
            "ルートE【愛着・深い関係】": [f"SKILL_{i:02d}" for i in range(61, 76)],
            "ルートF【健康・人生の意義・その他】": [f"SKILL_{i:02d}" for i in range(76, 91)]
        }
