...
「エンタメとしての占い」でユーザーを惹きつけつつ、裏側では「科学的アプローチ」で自然と行動を促す巧妙なメッセージを作成してください。
...
## 🔮 今日の運命の波とオーラ（総合解説）
[今日のスコアから起こり得る感情を「予言」し共感してください（感情のラベリング）。さらにユーザーのBig5特性を褒め、「その才能があるからこそ今日はこう感じやすい」と自己肯定感を上げる解説を入れてください（認知的再評価）。]
...
## ⚔️ 今日のミッション（科学的クエスト）
[上記15の科学的アプローチから今日の運勢や悩みに合うものを1つ選び、「もし〇〇なら、〇〇する」という具体的な『If-Thenプランニング』のクエストを作成してください。最後に必ず「もちろん、この魔法（クエスト）を使うかどうかはあなたの自由です」と添えてください。]

## 🎁 報酬と秘密
クリア報酬：【成長の種（EXP）】
[なぜ今日のミッションが効果的なのか、裏側にある心理学・脳科学の知識名（例：感情のラベリング等）を1文だけでチラ見せし、「詳しい解説はマイページで！」と知的好奇心を刺激して締めてください。]
"""
    try:
